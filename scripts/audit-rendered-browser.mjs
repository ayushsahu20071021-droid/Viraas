...
for (const world of ['Garba', 'Navratri', 'Diwali', 'Festive Party', 'College Fest']) { const count = couples.filter((c) => c.occasions?.[0] === world).length; if (count !== 20) failures.push(`${world} expected 20 Couple Edit looks, got ${count}`) }
...
if (couples.some((c) => !/\.jpg$/i.test(c.imageUrl || '') || !/\/images\/couples-v2\//.test(c.imageUrl))) failures.push('a Couple Edit look does not use a couples JPG')
